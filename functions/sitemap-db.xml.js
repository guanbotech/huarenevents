...
export async function onRequestGet({ env }) {
  const [articles, cities, platforms] = await Promise.all([
    queryRows(env, `SELECT slug, updated_at AS updatedAt, created_at AS createdAt FROM articles WHERE status = 'published' ORDER BY datetime(updated_at) DESC LIMIT 500`),
  ...
  const urls = [
    ...articles.map((item) => urlItem(`/news/${item.slug}`, item.updatedAt || item.createdAt, "0.70")),
